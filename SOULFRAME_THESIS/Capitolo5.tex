...
A livello teorico, la scelta degli indicatori richiama il quadro di letteratura su ECA/Voice UX presentato nel Capitolo~\ref{chap:stato-arte}, soprattutto \autoref{sec:eca-xr} e \autoref{sec:pipeline-ai}.
...
I meccanismi tecnici richiamati nei tre casi sono analizzati in dettaglio nel Capitolo~\ref{chap:implementazione}, soprattutto \autoref{subsec:rag-memory}, \autoref{subsec:tts-streaming} e \autoref{subsec:error-retry-fallback}.
...
Il limite più evidente resta il tempo al primo audio TTS (Tabella~\ref{tab:tempi-fase}), mentre la latenza end-to-end completa è ancora n.d. sul piano quantitativo. La fase RAG/LLM resta il fattore meno isolato: dipende dal runtime locale di inferenza (Ollama), dal profilo del modello quantizzato e dalla disponibilità di risorse. Nel deploy su server si sommano overhead di rete e proxy, con jitter più marcato nei casi lenti e maggiore probabilità di timeout lato client.

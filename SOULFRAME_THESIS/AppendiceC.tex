...
Questo disegno evita che l'utente entri direttamente in conversazione con un profilo ancora incompleto. In questo modo onboarding e bootstrap restano parte reale del sistema, non un prologo separato.
...
\]
dove \(s_{\mathrm{seq}}\) deriva dalla distanza di Levenshtein normalizzata e \(s_{\mathrm{word}}\) dall'overlap lessicale tra i token. La soglia usata nel progetto è \texttt{setupVoiceMinSimilarity = 0.7f}. Non serve a certificare l'identità del parlante: serve a scartare reference vocali troppo corte, troppo rumorose o troppo lontane dalla frase proposta.
...
Da qui entrano in gioco anche i componenti che rendono l'avatar meno ``piatto'' durante i turni. In SOULFRAME il comportamento embodied non sta a lato di \texttt{MainMode}: resta agganciato allo stesso ciclo conversazionale e ne segue gli stati.

{\subsectionfont{\upshape}\paragraph{\normalfont\texttt{AvaturnULipSyncBinder}.}}
Il binder non assume un naming unico dei blendshape. Cerca invece a runtime candidati compatibili per i visemi principali e aggiorna i pesi in \texttt{LateUpdate}. Il motivo è pratico: gli avatar non sono tutti uguali, quindi un mapping statico sarebbe troppo fragile.
...
\begin{figure}[H]
    ...
    \caption{Schermata di \texttt{SetupVoice} con validazione fallita: la frase pronunciata non raggiunge la soglia minima di similarità, quindi il campione vocale non viene registrato.}

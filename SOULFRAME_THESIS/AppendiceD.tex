...
Qui il browser non fa da semplice contenitore della build Unity, ma prende in carico alcune funzioni che il runtime WebGL da solo non riesce a gestire bene: microfono, overlay Avaturn e ritorno del focus al canvas.
...
Overlay, callback e refocus non servono soltanto a chiudere bene l'editor avatar: evitano che DOM e canvas Unity si contendano tastiera, click e stato della pagina proprio durante un passaggio delicato dell'onboarding.

\section{Integrazioni browser e filesystem virtuale}
In questa seconda parte rientrano invece i plugin che servono a far passare file e stato del browser verso il client senza rompere il flusso dell'onboarding e della memoria.

\paragraph{\normalfont\texttt{WebGLFilePicker.jslib} e file picker browser.}
Il file picker WebGL serve soprattutto a \texttt{SetupMemory}. Il plugin apre il selettore di file del browser, converte il contenuto in base64 e lo rimanda al client come piccolo JSON. Il frontend può così trattare in modo abbastanza uniforme selezione documento e selezione immagine, pur restando dentro i vincoli della pagina web.
...
\caption[Overlay Avaturn in WebGL]{Overlay del configuratore Avaturn aperto nel flusso WebGL di SOULFRAME. Il client Unity resta sospeso finché il browser non restituisce l'avatar selezionato tramite callback.}

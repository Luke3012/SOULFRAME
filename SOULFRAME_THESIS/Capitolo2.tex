...
La traiettoria che porta dai CA scriptati ai sistemi basati su modelli linguistici è sintetizzata in Figura~\ref{fig:ca-evoluzione}. La figura serve a chiarire perché embodiment e gestione del dialogo vengano oggi trattati insieme nei prototipi XR.
...
La figura funziona qui come mappa di lettura: i tre blocchi vengono ripresi subito sotto con vincoli e problemi diversi.
...
Whisper è un modello STT basato su architettura Transformer e addestrato su larga scala con supervisione debole. L'addestramento su circa 680.000 ore e l'impostazione multilingue (99 lingue) spiegano perché venga spesso considerato robusto in zero-shot rispetto a variazioni di voce e contesto acustico.\cite{Radford2023Whisper} In una pipeline conversazionale questo aspetto conta molto: un errore in trascrizione non resta confinato allo STT, ma si propaga alla memoria e alla generazione della risposta. La Figura~\ref{fig:whisper-arch-cap2} richiama la separazione tra encoder acustico e decoder testuale su cui si appoggia questo uso del modello.
...
Più che entrare nel dettaglio del modello, qui la figura serve a collocare il punto di contatto tra segnale acustico e testo.
...
La query testuale viene trasformata in embedding, usata per recuperare i passaggi pertinenti dall'indice e poi reinserita nel contesto fornito al Large Language Model (LLM). La risposta non dipende quindi solo dalla memoria parametrica del modello, ma anche da una memoria non parametrica aggiornabile.\cite{Lewis2020RAG} In questo progetto il punto è importante perché la memoria deve restare separata per avatar e poter crescere nel tempo senza cambiare i pesi del modello. Figura~\ref{fig:rag-frontiers} mostra il flusso retrieval$\rightarrow$generation.
...
Il punto chiave è che RAG rende praticabile una memoria per-avatar esterna al modello e aggiornabile nel tempo. La figura serve soprattutto a tenere distinta questa separazione tra recupero del contesto e generazione, più che a esaurire le varianti possibili di RAG. La memoria può quindi evolvere intervenendo sui contenuti recuperabili, non sui pesi del modello.

\subsection{Text-to-Speech con Coqui XTTS v2}
Il terzo stadio riguarda la resa vocale dell'avatar. In un ECA embodied la risposta non deve essere solo pertinente sul piano testuale: timbro, lingua e continuità della voce influenzano direttamente la percezione dell'interlocutore. Per questo il progetto adotta Coqui XTTS v2.

Coqui XTTS v2 offre supporto multilingue e \emph{voice cloning} tramite un campione di riferimento, senza richiedere l'addestramento di un modello vocale dedicato per ogni utente.\cite{Casanova2024XTTS} Nel progetto questo significa poter associare una voce al singolo avatar e riutilizzarla nei turni successivi, mantenendo un'identità sonora coerente pur restando in uno scenario zero-shot. Figura~\ref{fig:xtts-arch-cap2} richiama l'architettura del modello.
...
Una pipeline locale a tre stadi risponde a un compromesso preciso tra latenza percepita, controllabilità del sistema e continuità del profilo avatar. Qui la modularità non è un fine astratto: serve a sostituire i singoli blocchi senza riscrivere il client e a mantenere separati problemi diversi, cioè trascrizione, memoria, generazione e voce.

Il posizionamento del progetto dipende anche da come viene trattato l'avatar. Qui non è un semplice asset grafico, ma un profilo che combina aspetto, voce e memoria, così che la conversazione non riparta da zero a ogni sessione. Ho quindi vincolato l'accesso alla conversazione operativa alla disponibilità di un profilo minimo, per evitare un'esperienza \textit{stateless}. L'esecuzione via browser abbassa la barriera di accesso rispetto a configurazioni VR più tradizionali, ma rende più stringenti i vincoli su audio, routing e latenza: per questo streaming, warmup e segnali di attesa diventano parte integrante della soluzione e non semplici ottimizzazioni.

...
RNF1 (Efficienza prestazionale - time behaviour): la latenza percepita deve restare controllata misurando il tempo tra rilascio del comando push-to-talk e primo campione audio riprodotto dal TTS. Per il prototipo si adotta come soglia prestazionale una risposta udibile entro 12 s in esecuzione locale e entro 16 s in deploy pubblico. Il target è calibrato sulle misure osservate di STT+TTS e include un margine operativo per la fase RAG/LLM, non misurata sistematicamente per singolo stadio. Questa metrica non coincide con il tempo di cold-start iniziale dei servizi. Qui la verifica ha carattere osservativo: i timestamp lato client sono usati per diagnosi e confronto tra ambienti, mentre una telemetria turn-based persistente è indicata come sviluppo necessario per una validazione quantitativa completa, come discusso nel Capitolo~\ref{chap:risultati}. Di conseguenza le soglie 12/16 s vanno lette come target operativo di progetto, non come benchmark statistico conclusivo. Approcci a micro-servizi per agenti sociali real-time trattano la latenza come vincolo primario e riportano tempi dell'ordine di pochi secondi fino alla prima emissione vocale in pipeline ottimizzate \cite{Lin2024Estuary}.
...
In deploy pubblico, l'elemento chiave di integrazione è il reverse proxy Caddy, con due funzioni: terminazione TLS e punto d'ingresso unico per il browser. Il client WebGL comunica con un solo origin HTTPS e invoca le API tramite path riscritti (\texttt{/api/whisper/*}, \texttt{/api/rag/*}, \texttt{/api/avatar/*}, \texttt{/api/tts/*}). Caddy inoltra le richieste alle porte interne 8001--8004, mantenute non esposte verso l'esterno.

La soluzione riduce la complessità lato browser (stessa origine, routing consistente) e isola i servizi, che restano indirizzabili e gestibili separatamente a livello di processo. Nel progetto Caddy è stato preferito a Nginx per tre motivi operativi: TLS automatico, redirect HTTP$\rightarrow$HTTPS integrato e configurazione più compatta in fase di bootstrap. In questo scenario i path \texttt{/api/*} non sono solo una convenzione utile: impediscono che una build pubblica tenti di contattare \texttt{127.0.0.1:800x} sul browser dell'utente finale, che in quel contesto punterebbe alla sua macchina e non al server che ospita SOULFRAME. Nel prototipo, gli endpoint non implementano autenticazione/autorizzazione applicativa e l'architettura assume una rete di esecuzione controllata. Figura~\ref{fig:arch-cap3} riassume questa vista a blocchi.
...
\begin{figure}[H]
\centering
\includegraphics[width=0.90\textwidth]{pipeline_e2e.png}
...
Le due figure svolgono ruoli diversi: la prima riassume il flusso logico della conversazione, la seconda mostra come quel flusso venga distribuito tra client, proxy e servizi.

La stessa scomposizione in componenti viene mantenuta sia in locale sia su server. In ambiente Windows lo script di avvio coordina l'esecuzione dei servizi e applica controlli di base (ad esempio la disponibilità delle porte) per ridurre conflitti tra processi durante lo sviluppo. In ambiente Ubuntu, ciascun micro-servizio è gestito come unità \texttt{systemd} (systemd: gestore di processi e servizi del sistema operativo Linux, responsabile dell'avvio automatico e del restart dei servizi), con comandi amministrativi che permettono start/stop/status e aggiornamenti senza dover riconfigurare manualmente l'intero stack. Il rovescio della medaglia è una complessità operativa maggiore rispetto a un eseguibile unico, ma nel prototipo questo costo è risultato accettabile perché i colli di bottiglia più difficili da diagnosticare emergevano tra servizi diversi.
...
Il flusso conversazionale end-to-end segue una pipeline a turni: parte dall'input vocale dell'utente e termina con la riproduzione della risposta sintetizzata, mantenendo sul client Unity WebGL le responsabilità di interazione e rendering e delegando al backend le fasi di inferenza. All'interno del blocco RAG, il router d'intento non si limita a classificare il turno, ma modifica anche il percorso interno con cui viene costruito il contesto della risposta. L'interazione è di tipo push-to-talk: l'utente tiene premuto \texttt{SPACE} per parlare e, al rilascio, il client chiude la registrazione e prepara un file audio in formato WAV, includendo un parametro di lingua nella richiesta. In ambiente WebGL l'acquisizione del microfono e la gestione del contesto audio del browser richiedono un bridge JavaScript, realizzato qui tramite il plugin \texttt{AudioCapture.jslib} richiamato dal componente di registrazione in Unity.\footnote{Unity Technologies, \emph{Audio in WebGL}, \url{https://docs.unity3d.com/6000.2/Documentation/Manual/webgl-audio.html}} Nel browser, infatti, permessi microfono, \texttt{AudioContext} e buffer audio restano sotto controllo JavaScript.
...
SOULFRAME introduce inoltre strategie specifiche per mitigare i tempi morti più evidenti. Per contenere la latenza iniziale, il servizio TTS adotta meccanismi di warmup durante il boot; in parallelo, il frontend mostra un pannello di caricamento dedicato e abilita il flusso operativo solo dopo i controlli di bootstrap su Coqui e RAG, seguiti dalla verifica di voce e memoria dell'avatar attivo. Lo stesso problema può riemergere durante i turni successivi, quando la pipeline STT$\rightarrow$RAG/LLM$\rightarrow$TTS è in corso: in questa fase il sistema usa un fallback audio di cortesia per evitare silenzi prolungati e dare un segnale immediato di reattività. La Figura~\ref{fig:ui-state-diagram-cap3} contestualizza questo comportamento nel ciclo degli stati UI. Warmup, wait phrases e streaming sono approfonditi nel Capitolo~\ref{chap:implementazione}.

Il frontend, però, non attende sempre lo stesso artefatto audio. Le \emph{wait phrases} sono brevi clip già associate all'avatar dopo il setup voce oppure generate on demand dal backend se mancanti; la risposta principale, invece, arriva come stream WAV progressivo. Si distinguono così un feedback immediato e a basso costo e una sintesi più lenta ma semanticamente completa.
...
Figura~\ref{fig:flusso-e2e-cap3} sintetizza la sequenza temporale e i formati dati scambiati tra componenti, evidenziando le trasformazioni principali (WAV$\rightarrow$testo, testo$\rightarrow$testo contestualizzato, testo$\rightarrow$audio). Nel blocco RAG la figura distingue i tre passaggi essenziali: capire la richiesta, recuperare il contesto utile e controllare la risposta finale. Mostra anche che, in deploy con reverse proxy, gli endpoint REST restano invariati a livello logico e vengono raggiunti tramite path riscritti sotto un unico origin HTTPS.
...
Nel percorso locale, il client seleziona un profilo già disponibile e lo carica direttamente in scena. Nel percorso importato, invece, il profilo viene creato nell'overlay web di Avaturn, il browser restituisce a Unity un payload JSON con URL di export e metadati minimi, e il client delega al backend l'import del \texttt{.glb}. Il servizio asset scarica il file, lo inserisce nel catalogo persistente e restituisce un \texttt{cached\_glb\_url}; solo a quel punto il client scarica il modello dal proprio backend e lo istanzia in scena. In entrambi i casi la convergenza è la stessa: selezione del profilo, caricamento del modello e attivazione dell'avatar nella pipeline conversazionale.

Dopo selezione o creazione, il flusso prosegue con onboarding voce e memoria per-avatar. L'accesso a \texttt{MainMode} avviene solo quando i prerequisiti minimi del profilo risultano soddisfatti, così da evitare conversazioni prive di contesto operativo. La presenza di modelli locali di fallback è importante soprattutto quando il backend avatar non è raggiungibile: la libreria non collassa su una schermata vuota, ma perde solo il ramo di import dinamico. Il compromesso è chiaro: delegare il catalogo al backend aumenta la dipendenza dalla rete, ma evita che persistenza browser, cache locali e catalogo degli avatar importati divergano tra loro. La Figura~\ref{fig:flusso-avatar-cap3} esplicita questo raccordo tra percorso locale/importato e convergenza su setup conversazionale. Bridge WebGL, ciclo di import e logiche di robustezza lato asset sono approfonditi nel Capitolo~\ref{chap:implementazione}.
...
\hline
RAG/LLM & \path{rag_server.py} & \texttt{8002} & \path{GET /health}\ \path{GET /avatar_stats}\ \path{POST /chat}\ \path{POST /chat_session/start}\ \path{POST /remember}\ \path{POST /recall}\ \path{POST /ingest_file}\ \path{POST /describe_image} & Orchestrazione della risposta contestuale: memoria per-avatar su ChromaDB, classificazione dell'intento, ricerca mista lessicale+semantica, filtraggio dei passaggi più affidabili, controllo finale del testo e metadati di turno (\texttt{intent}, \texttt{rag\_used}, \texttt{auto\_remembered}). \\
\hline
...
Whisper copre la trasformazione audio$\rightarrow$testo e costituisce il punto di ingresso del canale vocale utente. Il servizio RAG/LLM governa memoria per-avatar e generazione contestuale; il TTS converte la risposta in audio, riusando profili vocali per-avatar e gestendo le frasi di attesa; l'Avatar Asset Server separa import, cache e distribuzione dei modelli \texttt{.glb} dal resto della pipeline. In questa sottosezione, il termine \emph{ricerca ibrida} indica la combinazione di similarità vettoriale e BM25 nella fase di retrieval del RAG; nel prototipo il bilanciamento 0.6/0.4 favorisce leggermente BM25 perché le query utente contengono spesso keyword e nomi propri, lasciando comunque spazio al recupero semantico di ricordi parafrasati.

La scomposizione in servizi autonomi mantiene il client Unity disaccoppiato dalla logica interna di inferenza e storage, e permette di aggiornare STT, RAG o TTS senza cambiare il flusso utente. In Figura~\ref{fig:rag-swagger-cap3} è riportata un'istantanea della documentazione automatica degli endpoint; aiuta a visualizzare la granularità reale delle route, mentre il contratto operativo resta quello descritto nel testo e nella Tabella~\ref{tab:microservizi-cap3}.
...
Nel sistema, la persistenza dei dati di profilo è organizzata su filesystem e separata per \texttt{avatar\_id}: ogni avatar mantiene tre store dedicati, cioè \texttt{avatar\_store/} per gli asset \texttt{.glb}, \texttt{voices/avatars/} per il profilo vocale e \texttt{rag\_store/} per la memoria conversazionale persistente. In questo modo identità dell'avatar e risorse operative restano allineate lungo tutto il ciclo di vita del profilo. La Figura~\ref{fig:persistenza-tree-cap3} mostra la distribuzione dei tre store persistenti e i punti di verifica disponibili.
...
In ambiente Ubuntu il deploy è orientato all'esposizione pubblica controllata del prototipo. I micro-servizi restano su rete interna, mentre il browser raggiunge un unico origin HTTPS tramite reverse proxy. Questo semplifica l'integrazione lato client (stessa origine, path applicativi uniformi) e riduce la superficie di esposizione diretta delle singole porte interne.
...
Accanto ai micro-servizi applicativi, il prototipo include un livello di supporto operativo che copre tre esigenze: lifecycle dei processi, gateway/reverse proxy e automazione amministrativa.

...
- `RAG_LOG_DIR` (default generato in env: `/home/<utente_runtime>/soulframe-logs`, fallback: `/opt/soulframe/backend/log`)
...
- non viene eseguita migrazione automatica dei log già esistenti nel path precedente.

### Cartella update automatica

`sfadmin` usa `UPDATE_DIR` con questa priorità:
...
- altrimenti il file ZIP più recente trovato in `UPDATE_DIR`.

### Pulizia file update dopo conferma

Dopo update riuscito:

- può eliminare il file ZIP sorgente;
- può eliminare i file sorgente backend/setup applicati.
...
Lo script estrarrà `soulframe_update/` automaticamente, copierà i file in `/opt/soulframe/SOULFRAME_SETUP`,
si rilancerà da lì e continuerà il setup.

## Permessi e line ending (CRLF/LF)

`/opt/soulframe` e i file sotto `/opt` sono normalmente di proprietà `root`.
Per modificare o installare file in quelle cartelle, usa `sudo`.

Lo script prova già a normalizzare automaticamente i line ending degli script Linux.
